...
const ListCaixas = ({ estabelecimentoId, apenasFechados = false, searchQuery = '', onVerDetalhes }) => {
  ...
  const fetchCaixas = useCallback(async () => {
    ...
    try {
      const response = await api.get(`/caixas/${estabelecimentoId}?fechados=${apenasFechados}`);
      if (response.success) {
        setCaixas(response.data.caixas || []);
      } else {
        setError(response.message || 'Erro ao carregar caixas');
      }
    } catch (err) {
  ...
  }, [estabelecimentoId, apenasFechados]);

  // Buscar caixas quando o componente montar ou parâmetros mudarem
  useEffect(() => {
    fetchCaixas();
  }, [fetchCaixas]);
    ...
    return (
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-8 min-h-[40vh] flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={fetchCaixas} className="px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-colors">
            Tentar novamente
          </button>
        </div>
      </div>
