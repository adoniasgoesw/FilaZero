...
const ListHistoricoPedidos = ({ estabelecimentoId, caixaId, onVerDetalhes, onPedidosLoaded }) => {
  ...
  // Função para buscar histórico de pedidos
  const fetchHistoricoPedidos = useCallback(async () => {
    ...
    try {
      const response = await api.get(`/historico-pedidos/${estabelecimentoId}?caixa_id=${caixaId}`);
      if (response.success) {
        setItems(response.data || []);
      } else {
        setError(response.message || 'Erro ao carregar histórico de pedidos');
      }
    } catch (err) {
  ...
  // Buscar histórico quando o componente montar ou parâmetros mudarem
  useEffect(() => {
  ...
  if (error) return (
    <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-8 min-h-[40vh] flex items-center justify-center">
      <div className="text-center">
        ...
        <button onClick={fetchHistoricoPedidos} className="px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-colors">Tentar novamente</button>
      </div>
    </div>
  );
